...
function provision_storage () {
   ...
   # Make a SWAP space available and update fstab
   mkswap /dev/xvdx
   swapon /dev/xvdx
...
function install_grid () {
   #
   # only needed once as EBS is attached to new ec2 instance
   #
   if [[ ! -f /stage/grid_install_complete ]] ; then
